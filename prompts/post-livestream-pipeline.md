...
```
~/bago-strategy/auto_shorts.sh <YouTube連結> content_MMDD
```
產出：
- ~/bago-strategy/content_MMDD/audio.txt（逐字稿）
- ~/bago-strategy/content_MMDD/audio.srt（帶時間戳）
- ~/bago-strategy/content_MMDD/clips.txt（AI 挑的精華片段）

### Step 2：短影片剪輯
clips.txt 產出後，review 確認沒問題，再跑一次 auto_shorts.sh 同樣連結剪出影片。
產出：~/bago-strategy/content_MMDD/shorts/ 裡的直式短影片
...
所有內容產出到：~/bago-strategy/content_MMDD/content-factory/
- shorts-scripts.md
- golden-quotes.md
- line-push-content.md

### Step 3.5：擴充內容銀行
從新的逐字稿萃取金句和教學要點，追加到 `~/bago-strategy/shorts-web/content_bank.json`：
- 10 則金句（有衝擊力、反直覺、不超過 30 字）
- 5 個教學主題（每個含 topic + 3 個 points，給一半留一半）
- 分類：技術觀念 / 收入見證 / 行業現實 / 痛點共鳴 / 學員故事
- 不重複已有的金句和主題
- 不用「治療」，用「調整」「處理」「恢復」
...
追加完後部署到 Zeabur：
```
cd ~/bago-strategy/shorts-web && npx zeabur@latest deploy --project-id 69bd3b86e0c2caf3a5f8ee3f --service-id 69c612fe82fb34707a9f4c8a --json
```
...
產出到：~/bago-strategy/content_MMDD/chase-sequences/
...
### Step 5：整理摘要
產出一份總覽給你 review：
~/bago-strategy/content_MMDD/SUMMARY.md
